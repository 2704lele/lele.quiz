...
import { checkSimplifiedChinese, checkPinyinSyllables, checkVietnameseMeaning, normalizeTopicString } from "./gatekeeper.js";
...
async function callGeminiAPI(apiKey, requestedModel, systemPrompt, userPrompt, config = {}) {
  ...
  const gatewayName = config.aiGatewayName || "lelevocabcnquiz";
...
export function validateTopicUniquenessAndQuality(topicItem, history = {}) {
  ...
  if (/(v\.v\.|v\/v|\betc\b)/i.test(rawTopicTitle)) {
    errors.push(`Chủ đề '${rawTopicTitle}' chứa ký hiệu liệt kê (v.v., etc...). Phải là 1 chủ đề rõ ràng.`);
  }

  // 2. Negative Context: Topic Recurrence Check
  const normTopic = normalizeTopicString(rawTopicTitle);
  const isRecentTopic = recentTopics.some(rt => {
    const normRt = normalizeTopicString(String(rt));
    return normRt && normRt === normTopic;
  });
  if (isRecentTopic) {
    errors.push(`Chủ đề '${rawTopicTitle}' vừa mới xuất hiện trong lịch sử tab 'vocabCN'. Hãy chọn chủ đề khác!`);
  }
  ...
  // 4. Rule 1: 100% Simplified Chinese Check
  const c1 = checkSimplifiedChinese(words);
  if (!c1.passed) {
    errors.push(...c1.violations);
  }

  // 5. Rule 3: 100% Vietnamese Meaning Check (No English words)
  const c3 = checkVietnameseMeaning(words);
  if (!c3.passed) {
    errors.push(...c3.violations);
  }

  // 6. Rule 4: 1:1 Pinyin Syllables & Tone Marks Check
  const c4 = checkPinyinSyllables(words);
  if (!c4.passed) {
    errors.push(...c4.violations);
  }

  // 7. Rule 5: Internal Duplicates Check within the batch
  const newHanziList = [];
  ...
  // 8. Negative Context: Strict Pair Overlap Check against ALL Past Batches in History
  ...
  // 9. Semantic Relevance Check (Chặn các trường hợp vô lý "râu ông nọ cắm cằm bà kia")
