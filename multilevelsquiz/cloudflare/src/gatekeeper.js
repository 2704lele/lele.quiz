...
export function auditIdeaDeterministic(idea, historyOrPastBatches = []) {
  ...
  if (Array.isArray(historyOrPastBatches)) {
    ...
    recentTopics = (historyOrPastBatches.allTopics && historyOrPastBatches.allTopics.length > 0)
      ? historyOrPastBatches.allTopics
      : (historyOrPastBatches.recentTopics || pastBatches.map(b => b.topic).filter(Boolean));
  }
