/**
 * GitHub Actions Workflow Dispatch Client for VocabCNQuiz Pipeline
 * Triggers repository workflows directly from Cloudflare Worker with Zero-Secret dynamic parameters.
 */

/**
 * Trigger Ideation Workflow (vocabcn_ideation.yml)
 */
export async function triggerGitHubIdeationWorkflow(env, options = {}) {
  ...
  const workflow = env.GITHUB_IDEATION_WORKFLOW_FILE || env.GITHUB_IDEATION_WORKFLOW || "vocabcn_ideation.yml";
  ...
  const webhookUrl = options.cf_webhook_url || env.CF_WEBHOOK_URL || "https://lele-vocabcnquiz.hothihuong113.workers.dev/api/receive-ideas";
...
/**
 * Trigger Video Render Workflow (vocabcn_render.yml)
 */
export async function triggerGitHubRenderWorkflow(env, options = {}) {
  const owner = env.GITHUB_REPO_OWNER || "naadld";
  const repo = env.GITHUB_REPO_NAME || "lele2vid";
  const workflow = env.GITHUB_WORKFLOW_FILE || "vocabcn_render.yml";
  const token = env.GITHUB_TOKEN;
...
/**
 * Trigger Auto-QC Physical Video Inspection Workflow (vocabcn_qc.yml)
 */
export async function triggerGitHubQCWorkflow(env, options = {}) {
  const owner = env.GITHUB_REPO_OWNER || "naadld";
  const repo = env.GITHUB_REPO_NAME || "lele2vid";
  const workflow = env.GITHUB_QC_WORKFLOW_FILE || env.GITHUB_QC_WORKFLOW || "vocabcn_qc.yml";
  const token = env.GITHUB_TOKEN;
