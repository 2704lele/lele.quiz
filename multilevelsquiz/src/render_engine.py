...
try:
    ...
    from multilevelsquiz.src.audio_generator import ensure_all_sound_effects
except ImportError:
...
def render_manim_scene(
    ...
    # Discover manim binary or fallback to python -m manim
    python_bin = shutil.which("manim")
    if not python_bin or not os.path.exists(python_bin):
        for candidate in [
            "/usr/local/bin/manim",
            os.path.expanduser("~/.local/bin/manim"),
            os.path.join(os.path.dirname(sys.executable), "manim"),
            os.path.join(config.base_dir, ".venv", "bin", "manim"),
            os.path.join(os.path.dirname(config.base_dir), ".venv", "bin", "manim")
        ]:
    ...
    cmd_str = ' '.join(cmd)
    logger.info(f"Executing Manim render: {cmd_str}")
    print(f"🎬 [RenderEngine] Running Manim: {cmd_str}", flush=True)
    ...
    if result.returncode != 0:
        print(f"❌ [RenderEngine] Manim failed (code {result.returncode}):\n{result.stderr}", flush=True)
    ...
    if os.path.exists(bg_video_path):
        ...
        mux_cmd = [
            ...
            "-i", raw_video,
            "-filter_complex",
            "[0:v]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,setsar=1,fps=60[bg];"
