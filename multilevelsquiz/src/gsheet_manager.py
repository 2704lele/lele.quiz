...
class GSheetManager:
    ...
    def parse_level_entry(self, raw_str: str, level_num: int) -> Dict[str, Any]:
        """
        Parse 'hanzi | pinyin | han_viet | meaning_vi | nuance_note | visual_action'
        """
        if not raw_str:
            return {
                "level": level_num,
        ...
        return {
            ...
            "han_viet": parts[2] if len(parts) > 2 else "",
            "meaning_vi": parts[3] if len(parts) > 3 else (parts[0] if len(parts) > 0 else ""),
            "nuance_note": parts[4] if len(parts) > 4 else "",
            "visual_action": parts[5] if len(parts) > 5 else ""
        }
    ...
    def append_or_insert_batch(self, batch_data: Dict[str, Any], target_row: int = None) -> int:
        ...
        logger.info(f"Wrote multilevels batch #{target_row} at row {target_row}")
        return target_row
