...
export const VIETNAMESE_VALID_WORDS_WHITELIST = new Set([
  ...
  "game", "games", "laptop", "smartphone", "bus", "taxi", "fan", "fans",
  "cafe", "café", "cà phê", "bia", "so-mi", "sơ-mi", "ca", "ga", "pin",
  "xăng", "môtô", "mo-to", "buýt", "xe buýt", "link", "links", "post", "page", "fanpage"
]);
...
export async function processGatekeeperIdea(env, config, payload) {
  ...
  if (auditResult.passed) {
    ...
    try {
      ...
      console.log(`[GATEKEEPER] ⚡ Auto-triggered Render.yml for row #${finalBatchId}: ${renderMsg}`);
    } catch (rErr) {
    ...
    return {
      ...
      message: "Gatekeeper 1 passed. Idea batch verified on Google Sheet and auto-triggered Render.yml."
    };
  }
