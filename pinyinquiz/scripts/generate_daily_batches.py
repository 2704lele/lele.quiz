...
DEFAULT_WEBHOOK_URL = os.getenv("CF_WEBHOOK_URL", "https://lele-pinyinquiz.hothihuong113.workers.dev/api/receive-ideas")
...
def run_batch_mode(
    ...
    # Final summary (Gatekeeper Worker handles Telegram notifications upon actual verified Sheet persistence)
    logger.info("\n" + "=" * 50)
    ...
    if generated_rows_summary:
        for r in generated_rows_summary:
            logger.info(f"  • Row #{r[0]}: {r[1]} ({r[2]}) - Verified on Sheet")
    logger.info("=" * 50)
...
def run_single_row_mode(
    ...
    logger.info(f"✨ Single Row #{clean_row_id} generated. Gatekeeper will audit, heal, persist, and send Telegram notification upon verified write.")
...
def main():
    ...
    parser.add_argument("--update-sheet", action="store_true", default=True, help="Also write/update rows to Google Sheet directly (default: True)")
    parser.add_argument("--no-sheet", action="store_true", default=False, help="Do not write to Google Sheet")
