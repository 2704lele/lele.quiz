...
def run_auto_qc(target_row_id: str = None):
    ...
    for batch in video_batches:
        ...
        if result["passed"]:
            logger.info(f"✨ Batch #{row_id} PASSED all QC checks! Promoting Status to 'Ready'.")
            passed_count += 1
            ...
            try:
                ...
                logger.warning(f"Could not update status to Ready: {e}")
    ...
    if len(video_batches) > 0:
        ...
        summary_msg = (
            f"🛡️ <b>[VocabCN Auto-QC Gatekeeper Hoàn Tất]</b>\n"
            f"━o0o━\n\n"
...
def main():
    ...
    parser.add_argument("--row-id", "--row_id", dest="row_id", type=str, default=None, help="Inspect a specific batch ID")
