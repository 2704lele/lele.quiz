/**
 * LeLe Chinese Quiz - Ultra-Lightweight Edge Gateway & Router (Vocab CN Quiz)
...
export default {
  async fetch(req, env, ctx) {
    ...
    if (method === "GET" && url.pathname === "/") {
      return json({ project: "LeLe VocabCN Quiz Edge Router", pipeline: config.sheetTabName, status: "Online", architecture: "Ultra-Lightweight Async Dispatch (< 2ms)", timestamp: new Date().toISOString() });
    }
    ...
    const getParams = async () => {
      ...
      return {
        ...
        count: b.count || url.searchParams.get("count") || "1",
        level: b.level || url.searchParams.get("level") || "HSK 1",
