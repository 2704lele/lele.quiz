...
def generate_social_metadata(topic: str, level: str, words: List[Dict[str, str]]) -> Dict[str, Any]:
    ...
    yt_title = f"Thử Thách Đoán Nghĩa {level}: {clean_topic} {chosen_emoji} | Lê Lê Học Tiếng Trung #Shorts"
    if len(yt_title) > 95:
        yt_title = f"Đoán Nghĩa Tiếng Trung {level}: {clean_topic} {chosen_emoji} #Shorts"
    ...
    yt_description = f"""🎯 Thử thách đoán nghĩa Tiếng Trung ➔ Tiếng Việt {level} chủ đề: {clean_topic.upper()}!
...
"""
    ...
    tiktok_caption = f"Đố bạn dịch đúng 5/5 từ tiếng Trung chủ đề {clean_topic} này? {chosen_emoji} Thử ngay xem bạn được mấy điểm nhé! 💬👇 #lelehoctiengtrung #hoctiengtrung #tiengtrung #{level_tag.lower()} #vocabquiz #xuhuong #learnchinese"

    fb_caption = f"Thử tài phản xạ dịch Tiếng Trung sang Tiếng Việt {level} - Chủ đề {clean_topic}! {chosen_emoji}\nBạn tự tin đúng bao nhiêu câu? Comment kết quả bên dưới cùng Lê Lê nha! ✨\n\n#lelehoctiengtrung #tiengtrung #hoctiengtrung #{level_tag.lower()} #reelsvn #hsk"
