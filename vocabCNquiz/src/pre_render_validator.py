...
class PreRenderValidator:
    """
    Pre-render Linguistic and Negative Context Validator for VocabCNQuiz.
    Enforces the 5 Gatekeeper 1 Linguistic Rules and Negative Context Deduplication against Google Sheets tab 'vocabCN'.
    """
    ...
    def fetch_vocabcn_history(cls, gsheet_mgr: Optional[Any] = None) -> Dict[str, Any]:
        """
        Dynamic history loader that fetches all existing rows from tab 'vocabCN'.
        ...
        try:
            ...
            logger.info(f"Loaded history from Google Sheets: {len(recent_topics)} topics, {len(past_batches)} past batches.")
