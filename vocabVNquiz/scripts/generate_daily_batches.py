...
logger = logging.getLogger("VocabVNBatchGenerator")
...
def post_to_cloudflare_webhook(webhook_url: str, payload: Dict[str, Any], timeout: int = 120) -> Tuple[bool, Dict[str, Any]]:
    ...
    try:
        ...
        if status_code in (200, 201, 202) and (data.get("success", False) or data.get("dispatched", False) or data.get("status") == "accepted"):
            logger.info(f"✅ Webhook accepted idea #{payload.get('row_id')}: {data}")
            return True, data
        else:
            logger.warning(f"⚠️ Webhook response for idea #{payload.get('row_id')} (HTTP {status_code}): {data}")
            return False, data
    except Exception as e:
...
def format_single_batch_payload(
    ...
    """
    Format standard webhook payload and Sheet row representation for vocabVNquiz.
    """
    formatted_words = []
    sheet_word_cols = []
    words_for_metadata = []
...
def run_batch_mode(
    ...
    """
    Step 1: Batch Ideation Mode for VocabVN Quiz.
    ...
    logger.info(f"=== Starting Step 1 Batch Ideation (VocabVN): Count={count}, Keys={len(keys)} {masked_keys}, Delay={delay_seconds}s ===")
    ...
    for i in range(1, count + 1):
        ...
        if dry_run:
            ...
            if post_ok:
                ...
                generated_success_count += 1
                generated_rows_summary.append(sheet_row)
                logger.info(f"✅ Idea #{actual_row_id} successfully dispatched / processed.")
            else:
                err_msg = post_res.get("error") or post_res.get("message") or "Unknown error"
                logger.warning(f"⚠️ Webhook dispatch note for idea #{target_row_id}: {err_msg}")
                # Count as generated locally if fallback succeeded
                generated_success_count += 1
                generated_rows_summary.append(sheet_row)
...
def run_single_row_mode(
    ...
    """
    Step 2: Targeted Single-Row Re-generation Mode for VocabVN Quiz.
    ...
    logger.info(f"=== Starting Step 2 Single-Row Re-Gen for Row #{clean_row_id} (VocabVN) ===")
...
def main():
    parser = argparse.ArgumentParser(description="VocabVNQuiz Daily Batch Generator (Dynamic Gemini Models & Gatekeeper 1)")
