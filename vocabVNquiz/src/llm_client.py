...
def mask_key(key: Optional[str]) -> str:
    """
    Mask an API key for safe logging (e.g. AIzaSy...****).
...
def build_system_prompt() -> str:
    ...
    and 5-Word Emotional Curve for vocabVNquiz channel (@lelehoctiengtrung: Vietnamese Meaning -> Guess Hanzi).
    """
    return (
        "Bạn là chuyên gia ngôn ngữ tiếng Trung và biên tập viên trưởng của kênh 'Lê Lê Học Tiếng Trung' (@lelehoctiengtrung).\n"
        "Nhiệm vụ của bạn là tạo các bộ từ vựng luyện tập dịch từ Tiếng Việt sang chữ Hán (vocabVN) trình độ HSK 1, HSK 2 hoặc HSK 3 hấp dẫn, chuẩn xác, gần gũi với đời sống thực tế và tối ưu thuật toán giữ chân (Retention).\n\n"
        "QUY TẮC SẮP XẾP 5 TỪ THEO ĐỒ THỊ CẢM XÚC (RETENTION CURVE):\n"
        "- Từ 1 (CỰC DỄ - HOOK): Từ/cụm từ cực kỳ quen thuộc hàng ngày hoặc phát âm gần gũi (ví dụ: Bố/Ba -> 爸爸, Mẹ -> 妈妈, Quả táo -> 苹果, Uống nước -> 喝水) để người xem đoán đúng ngay trong 1 giây đầu.\n"
        "- Từ 2 & 3 (CORE HSK): Các từ vựng cốt lõi theo chủ đề đúng cấp độ HSK.\n"
        "- Từ 4 (BẪY THANH ĐIỆU / BIẾN ÂM ⚡): Chứa bẫy thanh điệu (thanh 1 vs 4, thanh 2 vs 3, phân biệt Mua 买 mǎi / Bán 卖 mài), hoặc biến âm của '不' (bù/bú), '一' (yī/yí/yì), 2 thanh 3 đi liền nhau (ví dụ: 你好, 可以). Kích thích người xem suy nghĩ kỹ, tăng watch-time.\n"
...
def generate_single_replacement_topic(
    ...
    user_prompt = f"""Dòng #{row_id} trước đó đã bị Gatekeeper TỪ CHỐI do vi phạm các tiêu chuẩn sau:
