/**
 * GitHub Actions Workflow Dispatch Client for VocabVNQuiz Pipeline
 * Triggers repository workflows directly from Cloudflare Worker with Zero-Secret dynamic parameters.
 */

/**
 * Trigger Ideation Workflow (vocabvn_ideation.yml)
 */
export async function triggerGitHubIdeationWorkflow(env, options = {}) {
  ...
  const workflow = env.GITHUB_IDEATION_WORKFLOW_FILE || env.GITHUB_IDEATION_WORKFLOW || "vocabvn_ideation.yml";
  ...
  // Fixed: removed hardcoded fallback to pinyinquiz
  const webhookUrl = options.cf_webhook_url || env.CF_WEBHOOK_URL || "https://lele-vocabvnquiz.hothihuong113.workers.dev/api/receive-ideas";
  ...
  const payload = {
    ...
    inputs: {
      ...
      level: String(options.level || "HSK 1"),
...
/**
 * Trigger Video Render Workflow (vocabvn_render.yml)
 */
export async function triggerGitHubRenderWorkflow(env, options = {}) {
  const owner = env.GITHUB_REPO_OWNER || "naadld";
  const repo = env.GITHUB_REPO_NAME || "lele2vid";
  const workflow = env.GITHUB_WORKFLOW_FILE || "vocabvn_render.yml";
  const token = env.GITHUB_TOKEN;
...
/**
 * Trigger Auto-QC Physical Video Inspection Workflow (vocabvn_qc.yml)
 */
export async function triggerGitHubQCWorkflow(env, options = {}) {
  const owner = env.GITHUB_REPO_OWNER || "naadld";
  const repo = env.GITHUB_REPO_NAME || "lele2vid";
  const workflow = env.GITHUB_QC_WORKFLOW_FILE || env.GITHUB_QC_WORKFLOW || "vocabvn_qc.yml";
  const token = env.GITHUB_TOKEN;
